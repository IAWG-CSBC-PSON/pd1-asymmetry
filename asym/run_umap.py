...
def run_umap(
    ...
    umap_data = pd.concat([mdata, umap_embedding_df], axis=1)

    ### write csv OR return pd data frame
    if save_path is not None:
        pd.DataFrame.to_csv(umap_data, save_path)
    else:
        return umap_data
